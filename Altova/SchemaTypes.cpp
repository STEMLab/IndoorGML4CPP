...
namespace altova
{
	...
	static unsigned int GetNumericValue(const string_type::const_iterator& x)
	{
		string_type::value_type v = *x;
		return v - '\x30';
	}

	inline unsigned int GetNumericValue(char_type c)
	{
		return c - '\x30';
	}
		...
		case 1:
			{
				...
				if (r.positive) 
				{
					if (r.overflow || r.value > static_cast<unsigned __int64>(std::numeric_limits<__int64>::max()))
						throw altova::OutOfRangeException(_T("string"), _T("long"), s);
					return (__int64)r.value;
				}
				else
				{
					if (r.overflow || r.value > 0 - static_cast<unsigned __int64>(std::numeric_limits<__int64>::min()))
						throw altova::OutOfRangeException(_T("string"), _T("long"), s);
					return -(__int64)r.value;
				}
			}
...
altova::Duration CoreTypes::CastToDuration(const string_type& s, const altova::Duration::ParseType pt)
{
	bool positive = true;
	DateTimeParseState state;
	state.pos = 0;
	...
	if (number)	// try 'Y'
	{
		...
		if (s.at(state.pos) == 'Y')
		{
			if (pt == Duration::DAYTIME)
				throw altova::StringParseException(_T("daytimeduration"), s);
			anyFound = true;
	...
	if (number) // try 'D'
	{
		...
		if (s.at(state.pos) == 'D')
		{
			if (pt == Duration::YEARMONTH)
				throw altova::StringParseException(_T("yearmonthduration"), s);
			anyFound = true;
