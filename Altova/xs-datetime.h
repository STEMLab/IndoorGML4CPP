...
namespace altova
{
	...
	public:
		...
		DateTime( const DateTime& dt ) : m_value( dt.m_value ), m_timezone( dt.m_timezone ) {}
		...
		ALTOVA_DECLSPECIFIER void SetTimezone(short tz) { m_timezone = tz; }
		ALTOVA_DECLSPECIFIER bool HasTimezone() const { return m_timezone != NO_TIMEZONE; }
		
		ALTOVA_DECLSPECIFIER int CompareTo(const DateTime& dt) const;
		...
		ALTOVA_DECLSPECIFIER int Weekday() const { return (int)(( m_value / TicksPerDay ) % 7); }
	...
	ALTOVA_DECLSPECIFIER bool operator!= (const DateTime& first, const DateTime& second);
	inline bool operator< (const DateTime& first, const DateTime& second) { return first.CompareTo(second) < 0; }
	inline bool operator> (const DateTime& first, const DateTime& second) { return first.CompareTo(second) > 0; }
	inline bool operator<= (const DateTime& first, const DateTime& second) { return first.CompareTo(second) <= 0; }
	inline bool operator>= (const DateTime& first, const DateTime& second) { return first.CompareTo(second) >= 0; }
