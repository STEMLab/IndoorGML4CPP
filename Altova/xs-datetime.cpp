...
namespace altova
{
	...
	int DateTime::CompareTo(const DateTime& dt) const
	{
		if (HasTimezone() == dt.HasTimezone())
		{
			__int64 val = NormalizedValue();
			__int64 otherval = dt.NormalizedValue();
			if(val < otherval) return -1;
			else if(val == otherval) return 0;
			else return 1;
		}
		else if (HasTimezone())
			return CompareTo(DateTime(dt.Value(), +14 * 60));
		else // (dt.HasTimezone())
			return DateTime(Value(), -14 * 60).CompareTo(dt);
	}
