...
static bool IsDefaultNamespace(const MSXML2::IXMLDOMNodePtr& pNode, const _bstr_t& uri)
{
	...
	if (attrs != 0)
	{
		...
		for (MSXML2::IXMLDOMNodePtr att = attrs->nextNode(); att != 0; att = attrs->nextNode())
			if (att->prefix == _bstr_t(_T("")) && att->baseName == _bstr_t(_T("xmlns")))
				if (_bstr_t( att->nodeValue ) == uri)
					return true;
	}
	if (pNode->parentNode != 0)
		return IsDefaultNamespace(pNode->parentNode, uri);
...
_bstr_t FindPrefixForNamespace(const MSXML2::IXMLDOMNodePtr& node, const _bstr_t& uri)
{
	...
	if (node->GetnamespaceURI() == uri && node->Getprefix() != _bstr_t(_T("")))
...
{
	string_type prefix = _T("");
	if (*pMemberInfo->NamespaceURI)
	{
		if (pNode->nodeType == MSXML2::NODE_ELEMENT && !IsDefaultNamespace(pNode, pMemberInfo->NamespaceURI))
		{
			BSTR _nsURI = 0;
			if (!FAILED(pNode->get_namespaceURI(&_nsURI)) && _nsURI != 0 && _bstr_t(_nsURI, false) == _bstr_t(pMemberInfo->NamespaceURI))
			{
				BSTR _prefix = 0;
				if (!FAILED( pNode->get_prefix(&_prefix)) && _prefix != 0)
					prefix = _bstr_t(_prefix, false); // we prefer parent' prefix when child has same namespace (according to DOM spec. for lookupPrefix algorithm)
			}
			else
				prefix = FindPrefixForNamespace(pNode, pMemberInfo->NamespaceURI);
		}
	...
	string_type localName = prefix + pMemberInfo->LocalName;
	...
	MSXML2::IXMLDOMNodePtr pNewNode = pDoc->createNode(_variant_t((short)MSXML2::NODE_ELEMENT),
		localName.c_str(), pMemberInfo->NamespaceURI);
	pNode->appendChild(pNewNode);
	return pNewNode;
}


MSXML2::IXMLDOMNodePtr MsxmlTreeOperations::AddElement(const MSXML2::IXMLDOMNodePtr& pNode, string_type prefix, string_type localName, string_type namespaceURI)
{
	if (!prefix.empty())
		prefix += _T(":");
...
#else
		_bstr_t m = error.ErrorMessage();
		string_type msg = (string_type)(((TCHAR*)m) ? m : "unknown");
		throw altova::DataSourceUnavailableException(filename + ": COM error while loading: " + msg);
#endif
	}	
}
...
{
	SaveDocument(document, filename, prettyPrint, false );
}

void MsxmlTreeOperations::SaveDocument( const DocumentType& document, const string_type& filename, bool prettyPrint, bool omitXmlDecl )
{
	SaveDocument( document, filename, prettyPrint, omitXmlDecl, _T("UTF-8"), false, false );
}
...
{
	SaveDocument( document, filename, prettyPrint, false, encoding, bBigEndian, bBOM );
}

void MsxmlTreeOperations::SaveDocument( const DocumentType& document, const string_type& filename, bool prettyPrint, bool omitXmlDecl, const string_type& encoding, bool bBigEndian, bool bBOM )
{
	std::vector<unsigned char> bytes;
	SaveToBinary( bytes, document, prettyPrint, encoding, bBigEndian, bBOM, !omitXmlDecl );
...
{
	return SaveXml( document, prettyPrint, false );
}

string_type MsxmlTreeOperations::SaveXml(const DocumentType& document, bool prettyPrint, bool omitXmlDecl)
{
	try 
	{
...
{
	return SaveToBinary( document, prettyPrint, false);
}

std::vector<unsigned char> MsxmlTreeOperations::SaveToBinary(const DocumentType& document, bool prettyPrint, bool omitXmlDecl)
{
	std::vector<unsigned char> result;
	SaveToBinary( result, document, prettyPrint, _T(""), false, false, !omitXmlDecl );
	return result;
}
